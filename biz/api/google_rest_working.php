...
if (is_array($data)) {
    ...
    if ($share) {
        ...
        $share_count = sql_num_rows($share);
        if ($share_count > 0) {
            
            while($row = $share->fetch_assoc()) {
                $childPageIdxArray[] = $row['share_child_page_idx'];
                $shareDataArray[] = $row;
            }


            // 각 land_pg_idx에 대해 건수 조회
            //$pageCounts = array_fill_keys($childPageIdxArray, 0);
            if (count($childPageIdxArray) > 0) {
                //$inClause = implode(',', array_fill(0, count($childPageIdxArray), '?'));
                $inClause = implode(',', $childPageIdxArray);
                ...
                SELECT land_pg_idx
                 ...
                 WHERE land_pg_idx IN ($inClause) 
                   ...
                   AND use_yn = 'Y' 
                 GROUP BY land_pg_idx";
                $landingResult = sql_query($landingQuery);

                // 결과 처리
                while ($row = sql_fetch_array($landingResult)) {
                    $pageCounts[$row['land_pg_idx']] = $row['count'];
                }
            }

            // shareAllocations 배열 설정 및 업데이트
            $shareAllocations = [];
            $totalShares = 0;
            foreach ($shareDataArray as $data) {
                $pageIdx = $data['share_child_page_idx'];
                $currentCount = isset($pageCounts[$pageIdx]) ? $pageCounts[$pageIdx] : 0;
                $remaining = max($data['share_count'] - $currentCount, 0);
                $totalShares += $remaining;

                $shareAllocations[$pageIdx] = [
                    'total' => $data['share_count'],
                    'remaining' => $remaining,
                    'currentCount' => $currentCount,
                    'share_parent_ptn' => $data['share_parent_ptn'],
                    'share_parent_page_idx' => $data['share_parent_page_idx'],
                    'share_child_ptn' => $data['share_child_ptn'],
                    'share_child_page_idx' => $data['share_child_page_idx']
                ];
            }

            $newDataArrived = true;
            if ($newDataArrived && $totalShares > 0) {
                // 기존 distributeData 함수 로직을 이곳에 통합
                $lowestUsedPercentage = 100;
                $candidates = [];
                $highestTotal = 0;
            
                foreach ($shareAllocations as $pageIdx => $allocation) {
                    $usedPercentage = ($allocation['total'] - $allocation['remaining']) / $allocation['total'] * 100;
                    if ($usedPercentage < $lowestUsedPercentage) {
                        $lowestUsedPercentage = $usedPercentage;
                        $candidates = [$pageIdx];
                        $highestTotal = $allocation['total'];
                    } elseif ($usedPercentage == $lowestUsedPercentage) {
                        if ($allocation['total'] > $highestTotal) {
                            $candidates = [$pageIdx];
                            $highestTotal = $allocation['total'];
                        } elseif ($allocation['total'] == $highestTotal) {
                            $candidates[] = $pageIdx;
                        }
                    }
                }
            
                // 동률일 경우 총수량이 가장 많은 페이지 선택
                if (!empty($candidates)) {
                    $selectedIdx = $candidates[0];
                    foreach ($candidates as $candidateIdx) {
                        if ($shareAllocations[$candidateIdx]['total'] > $shareAllocations[$selectedIdx]['total']) {
                            $selectedIdx = $candidateIdx;
                        }
                    }
            
                    // 선택된 할당 정보를 사용
                    $allocatedPage = $shareAllocations[$selectedIdx];
                    //echo "Data allocated to Page: " . $allocatedPage . "\n";
            
                    $page_idx = $allocatedPage['share_child_page_idx'];
                    $ptn_idx = $allocatedPage['share_child_ptn'];
                    $inflow_path = 'divide';
                }
            }
