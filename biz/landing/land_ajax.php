...
echo json_encode($result);
  ...
  echo json_encode("삭제완료");
    ...
    // 3회 이상 틀리면 세션 정리 후 차단
    if ($_SESSION['ss_excel_auth_fail_cnt'] >= 3) {
        unset($_SESSION['ss_excel_auth_code']);
        unset($_SESSION['ss_excel_password_passed']);
        unset($_SESSION['ss_excel_password_passed_until']);

        echo json_encode([
            'status' => 'blocked',
            'message' => '인증번호를 3회 이상 틀렸습니다. 다시 시도해주세요.'
        ]);
        exit;
    }
    ...
    if (empty($_SESSION['ss_excel_auth_code']) || $_SESSION['ss_excel_auth_code'] != $user_code) {
        ...
        echo json_encode([
            'status' => 'fail',
            'message' => "인증번호가 일치하지 않습니다. (남은기회: {$remain}회)"
        ]);
        exit;
    }
    ...
    // 비밀번호 선행 통과 세션 확인
    if (
        empty($_SESSION['ss_excel_password_passed']) ||
        $_SESSION['ss_excel_password_passed'] !== true ||
        empty($_SESSION['ss_excel_password_passed_until']) ||
        $_SESSION['ss_excel_password_passed_until'] < time()
    ) {
        unset($_SESSION['ss_excel_auth_code']);
        unset($_SESSION['ss_excel_auth_fail_cnt']);

        echo json_encode([
            'status' => 'fail',
            'message' => '비밀번호 재확인 세션이 만료되었습니다. 다시 시도해주세요.'
        ]);
        exit;
    }
    ...
    if (empty($mb_hp) || !preg_match($pattern, $mb_hp)) {
        echo json_encode([
            'status' => 'fail',
            'message' => '등록된 연락처가 휴대전화가 아니거나 없습니다.'
        ]);
        exit;
    }
    ...
    // 비밀번호 선통과 세션 저장
    $_SESSION['ss_excel_password_passed'] = true;
    $_SESSION['ss_excel_password_passed_until'] = time() + 180;
    ...
    if ($result !== null) {
        ...
        $sms_msg_id = isset($result['msg_id']) ? $result['msg_id'] : "";
    }
