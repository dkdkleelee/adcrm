...
, b.land_ptn_idx
, e.ptn_nm
, ifnull (f_get_mb_name(c.pg_mb_ptn), c.pg_mb_ptn) as mb_ptn_name
, b.land_deptno
, c.pg_mb_emp 
, ifnull (f_get_mb_name(c.pg_mb_emp), c.pg_mb_emp) as mb_emp_name
, b.name 
...
left join {$g5['crm_partner']}  e on c.pg_ptn_idx = e.ptn_idx 
";
...
if ($member['mb_deptno'] != "9") {
    if ($member['mb_level'] <= 6) {
        ...
        if ($member['mb_level'] == 4) {
            $add_cont = "and b.land_empno = {$member['mb_no']}";
        } else {
            $add_cont = "and b.land_deptno = {$member['mb_deptno']}";
        }
    }
}
...
if ($stx) {
    ...
    switch ($sfl) {
        case "ptn_idx":
            $sql_search .= " b.land_ptn_idx = $stx";
            break;

        case "mb_no":
            $sql_search .= " b.land_empno = '$stx'";
            break;
        ...
        case "insert_date":
            ...
            $to   = substr($stx,10,10);
            $sql_search .= "b.$sfl between '{$from} 00:00:00.000' and '{$to} 23:59:59.999'";
            $stx = $from."~".$to;
            break;
...
if (!$sst) {
    //$sql_order = "order by b.land_idx desc";
    $sql_order = "order by b.land_idx desc";
} else {
...
select land_ptn_idx 
  ...
  left join {$g5['crm_page']}    b on b.land_pg_idx = b.page_idx
 where b.use_yn = 'Y'
 $add_cond
group by b.pg_uri
            ...
            <div class="col-12">
                <div class="card card-danger card-outline">
                    <div class="card-header">
                        <h3 class="card-title">파일업로드 DB 조회(<?php echo number_format($total_count) ?>)</h3>
                    </div>
                    <div class="card-body">
                                        ...
                                        <table id="tbl_land" class="table table-striped table-bordered dt-responsive nowrap landpg-font-size" style="width:100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>이름</th>
                                                    <th>핸드폰번호</th>
                                                    <th><?php echo $sfl == "ptn_idx" && $stx != "" ? get_sort_bootst('b.land_pg_idx', '', 'desc', $sst, $sod, '페이지') : "페이지" ?></th>
                                                    <th>파일명</th>
                                                    <th>고객사</th>
                                                    <th>코드</th>
                                                    <th>담당자</th>
