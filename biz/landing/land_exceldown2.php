...
if($curr_yn == "on") {
    ...
    if($rdo_data == "collect") {
        ...
        $mainSql = "
        select  b.pg_uri
              , group_concat(CONVERT(AES_DECRYPT(UNHEX(a.tel), 'withus_secret_key') USING UTF8)) as tel 
...
";
        ...
        for ($i = 1; $main = sql_fetch_array($mainList); $i++) {
            ...
            $row['pg_chk_name'] = $main['pg_chk_name'] == "1" ? explode (",", $main['name']) : "";
            $row['pg_chk_data1'] = $main['pg_chk_data1'] == "1" ? explode (",", $main['option1']) : "";
            $row['pg_chk_data2'] = $main['pg_chk_data2'] == "1" ? explode (",", $main['option2']) : "";
            $row['pg_chk_data3'] = $main['pg_chk_data3'] == "1" ? explode (",", $main['option3']) : "";
            $row['pg_chk_data4'] = $main['pg_chk_data4'] == "1" ? explode (",", $main['option4']) : "";
            $row['pg_chk_data5'] = $main['pg_chk_data5'] == "1" ? explode (",", $main['option5']) : "";
            $row['pg_chk_data6'] = $main['pg_chk_data6'] == "1" ? explode (",", $main['option6']) : "";
            $row['pg_chk_data7'] = $main['pg_chk_data7'] == "1" ? explode (",", $main['option7']) : "";
            $row['pg_chk_data8'] = $main['pg_chk_data8'] == "1" ? explode (",", $main['option8']) : "";
            $row['pg_chk_data9'] = $main['pg_chk_data9'] == "1" ? explode (",", $main['option9']) : "";
    ...
    header("Content-Disposition: attachment; filename={$partner_name}_{$append_title}엑셀2다운로드_".date("Ymd_Hms").".xls" );
