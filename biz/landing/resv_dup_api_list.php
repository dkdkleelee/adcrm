...
if($member['mb_level'] >= 5) {
    ...
    select mb_no
         , mb_name
         , ifnull((select count(*) from gnp_crm_landing sub where a.mb_no = sub.land_empno and sub.use_yn = 'D' and insert_date2 = curdate() group by land_empno) ,0) as today_cnt
            ...
            <div class="col-12">
                <div class="card card-danger card-outline">
                    <div class="card-header">
                        <h3 class="card-title">API중복 DB(IN) 조회(<?php echo number_format($total_count) ?>)</h3>
                    </div>
                    <div class="card-body">
                                            ...
                                            <tbody>
                                                <?php for ($i = 0; $row = sql_fetch_array($result); $i++) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="<?php echo G5_BIZ_URL ?>/landing/resv_dup_api_list?sfl=ptn_idx&stx=<?php echo $row['land_ptn_idx'] ?>" target="_self">
                                                        ...
                                                        <td>
                                                            <a href="<?php echo G5_BIZ_URL ?>/landing/resv_dup_api_list?sfl=pg_uri&stx=<?php echo $row['pg_uri'] ?>" target="_self">
                                                                <?php echo $row['pg_uri'] ?>
                                                            </a>
...
<script>
    $(function() {
        ...
        if(flag == "") {
            ...
            $('#search_dupdb').selectpicker('hide');
            $('#search_dupdb').attr("disabled"   , true);
        }
