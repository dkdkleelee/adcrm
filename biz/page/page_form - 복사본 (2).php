        ...
        <form name="pageForm" id="pageForm" action="./page_form_update" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
            ...
            <div class="card">
                ...
                <div id="div_aft_pop_area" class="card-body <?php echo $resultOne['pg_aft_ad_yn'] != "Y" ? "d-none" : "" ?>">
                    ...
                    <!-- Row 1: 카테고리 / 후팝업 타이틀 / 노출 시작일 / 노출 종료일 -->
                    <div class="row">
                        <div class="col-md-2">
                            <div class="form-group">
                                <label>카테고리</label>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>후팝업 타이틀</label>
                                <input type="text" name="pg_aft_ad_title" class="form-control"
                                    value="<?php echo htmlspecialchars($resultOne['pg_aft_ad_title']) ?>"
                                    placeholder="제목 입력">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label>노출 시작일</label>
                                <input type="date" name="pg_aft_ad_sdate" class="form-control"
                                    value="<?php echo $resultOne['pg_aft_ad_sdate'] ?>">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label>노출 종료일</label>
                                <input type="date" name="pg_aft_ad_edate" class="form-control"
                                    value="<?php echo $resultOne['pg_aft_ad_edate'] ?>">
                            </div>
                        </div>
                    </div>
            
                    <!-- Row 2: 후팝업 상세설명 / 이미지 업로드 -->
                    <div class="row">
                        <div class="col-md-7">
                            <div class="form-group mb-0">
                                <label>후팝업 상세설명</label>
                                <textarea name="pg_aft_ad_desc" class="form-control" rows="5"><?php echo htmlspecialchars($resultOne['pg_aft_ad_desc']) ?></textarea>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="form-group mb-0">
                                ...
                                <!-- 파일 선택: 항상 최상단 -->
                                <div id="pop_img_upload_box" class="mb-2">
                                    <div class="custom-file">
                                        <input type="file" class="custom-file-input" id="pg_aft_ad_img" name="pg_aft_ad_img"
                                            accept="image/png, image/jpeg" onchange="aft_pop_preview(this)">
                                        <label class="custom-file-label" for="pg_aft_ad_img" id="label_aft_pop_img">
                                            <?php echo $aft_pop_file ? '이미지 교체 (PNG, JPG)' : '이미지 파일 선택 (PNG, JPG)' ?>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <?php if ($aft_pop_file): ?>
                                <!-- 저장된 이미지: X버튼 없이 표시 -->
                                <div id="pop_img_existing_box">
                                    <?php $img_url = $aft_pop_file['file_path'] . '/' . $aft_pop_file['file_name']; ?>
                                    <img src="<?php echo $img_url ?>"
                                        style="max-width:100%; border:1px solid #ddd; border-radius:4px;"
                                        class="img-thumbnail">
                                </div>
                                <input type="hidden" name="aft_pop_file_idx" id="aft_pop_file_idx" value="<?php echo $aft_pop_file['file_idx'] ?>">

                                <?php else: ?>
                                <input type="hidden" name="aft_pop_file_idx" id="aft_pop_file_idx" value="0">
                                <?php endif; ?>

                                <!-- 새로 선택한 이미지 미리보기: X버튼 있음 -->
                                <div id="pop_img_new_preview" class="mt-2" style="display:none;">
                                    <div style="position:relative; display:inline-block; width:100%;">
                                        <img id="pop_img_new_preview_img" src=""
                                            style="max-width:100%; border:1px solid #ddd; border-radius:4px;"
                                            class="img-thumbnail">
                                        <button type="button" class="btn btn-danger btn-xs"
                                                style="position:absolute; top:5px; right:5px;"
                                                onclick="aft_pop_clear_new()">
...
// ── 파일 선택 시 브라우저 미리보기 (서버 저장 없음) ─────────────
function aft_pop_preview(input) {
    if (input.files && input.files[0]) {
        var file = input.files[0];
        document.getElementById('label_aft_pop_img').textContent = file.name;

        var reader = new FileReader();
        reader.onload = function (e) {
            // 기존 저장 이미지 숨기기
            var existingBox = document.getElementById('pop_img_existing_box');
            if (existingBox) existingBox.style.display = 'none';

            // 새 이미지 미리보기 표시 (X버튼 포함)
            document.getElementById('pop_img_new_preview_img').src = e.target.result;
            document.getElementById('pop_img_new_preview').style.display = 'block';
        };
...
// ── 새로 선택한 이미지 취소 ─────────────────────────────────────
function aft_pop_clear_new() {
    var fileInput = document.getElementById('pg_aft_ad_img');
    fileInput.value = '';
    document.getElementById('label_aft_pop_img').textContent = '이미지 파일 선택';
    document.getElementById('pop_img_new_preview').style.display = 'none';
    document.getElementById('pop_img_new_preview_img').src = '';
...
// ── 기존 이미지 제거 버튼 클릭 → 삭제 예약 + 업로드 UI 노출 ─────
function aft_pop_remove_existing() {
    if (!confirm('기존 이미지를 삭제하시겠습니까?')) return;
    // hidden 값을 0으로 → 서버에서 삭제 처리
    document.getElementById('aft_pop_file_idx').value = '0';
    document.getElementById('pop_img_existing_box').style.display = 'none';
    document.getElementById('pop_img_upload_box').style.display = 'block';
}
 
// ── 폼 submit 전 유효성 검사 ────────────────────────────────────
// 기존 폼의 submit 이벤트에 아래 로직을 추가하세요.
// 예: $('form').on('submit', function(e) { ... });
//
// 핵심 로직만 발췌:
function validate_aft_pop() {
    var yn = document.getElementById('pg_aft_ad_yn').checked;
    if (!yn) return true; // OFF면 검사 불필요
 
    var existingFileIdx = parseInt(document.getElementById('aft_pop_file_idx').value, 10);
    var newFile = document.getElementById('pg_aft_ad_img');
    var hasNew = newFile && newFile.files && newFile.files.length > 0;
 
    // 기존 이미지도 없고, 새 파일도 선택 안 했으면 막기
    if (existingFileIdx === 0 && !hasNew) {
...
// 폼 submit 이벤트에 validate_aft_pop() 연결 예시
// (실제 폼 id/selector에 맞게 수정)
// $(document).on('submit', '#frm', function(e) {
//     if (!validate_aft_pop()) { e.preventDefault(); return false; }
// });
