...
$aft_pop_files = []; // 신규등록시 빈 배열 ??? 이코드위치 여기맞아???
// 뱃지/스타일 기본값 초기화 (신규등록시 사용)
$badge_bg    = '#ef4444';
$badge_name  = '';
$badge_color = '#ffffff';
$title_color = '#1c1e21';
...
if ($w == '') {
    ...
    if ($resultOne['pg_aft_ad_yn'] == 'Y') {
        ...
        ");
        while ($af = sql_fetch_array($aft_files_res)) {
            $aft_pop_files[] = $af;
        }
    }
    ...
    // 뱃지/스타일 JSON 파싱
    $aft_badge = json_decode($resultOne['pg_aft_ad_badge'] ?? '{}', true) ?? [];
    $aft_style = json_decode($resultOne['pg_aft_ad_style'] ?? '{}', true) ?? [];
    $badge_bg    = $aft_badge['bg']    ?? '#ef4444';
    $badge_name  = $aft_badge['name']  ?? '';
    $badge_color = $aft_badge['color'] ?? '#ffffff';
    $title_color = $aft_style['titlebg'] ?? '#1c1e21';
    $desc_color  = $aft_style['descbg']  ?? '#65676b';
...
.color-picker-wrap input[type=color] {
    width:36px; height:30px; padding:0; border:1px solid #ced4da;
...
<section class="content">
    <div class="container-fluid">
        <form name="pageForm" id="pageForm" action="./page_form_update" method="post" enctype="multipart/form-data">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center">
                    ...
                    <label class="ml-4 switch mb-0">
                        <input type="checkbox" id="pg_aft_ad_yn" name="pg_aft_ad_yn" value="Y" class="switch-input"
                            <?php echo $resultOne['pg_aft_ad_yn'] == "Y" ? "checked" : "" ?>>
                ...
                <div id="div_aft_pop_area" class="card-body <?php echo $resultOne['pg_aft_ad_yn'] != "Y" ? "d-none" : "" ?>">
                    <input type="hidden" name="org_aft_ad_yn" value="<?php echo $resultOne['pg_aft_ad_yn'] ?>">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-7">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select id="pg_aft_ad_cat" name="pg_aft_ad_cat" class="form-control">
                                            <?php
                                            $pop_categories = ["인터넷.TV","개인회생","정수기","창업","정책자금","보험","렌트","기타"];
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>노출 섹션</label>
                                        <select id="pg_aft_ad_news_sec" name="pg_aft_ad_news_sec" class="form-control">
                                            <?php
                                            $news_sections = [
                                                'normal'  => '🍵일반 뉴스',
                                                'best'    => '🔥 베스트 뉴스',
                                            ...
                                            foreach ($news_sections as $val => $label) {
                                                echo '<option value="'.$val.'" '.get_selected($resultOne['pg_aft_ad_news_sec'],$val).'>'.$label.'</option>';
                                            }
                                            ?>
                                        </select>
                                    </div>
                                        ...
                                        <div class="mt-2">
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input"
                                                    id="pg_aft_ad_is_main" name="pg_aft_ad_is_main" value="Y"
                                                    <?= $resultOne['pg_aft_ad_is_main'] == 'Y' ? 'checked' : '' ?>>
                                                <label class="custom-control-label" for="pg_aft_ad_is_main">대표 지정</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row mb-1 align-items-center">
                                <div class="col-md-8">
                                    <label class="mb-0" style="font-size:.78rem;">후팝업 타이틀</label>
                                    <input type="text" name="pg_aft_ad_title" id="pg_aft_ad_title"
                                        class="form-control form-control-sm"
                                        value="<?= htmlspecialchars($resultOne['pg_aft_ad_title'] ?? '') ?>">
                                </div>
                                <div class="col-md-4">
                                    <label class="mb-0" style="font-size:.78rem;">타이틀 색상</label>
                                    <div style="display:flex;align-items:center;gap:5px;">
                                        <input type="color" id="title_color_picker" value="<?= h($title_color) ?>"
                                            style="width:32px;height:28px;padding:0;border:1px solid #ced4da;border-radius:4px;cursor:pointer;"
                                            oninput="document.getElementById('title_color_text').value=this.value;syncStyleJson();">
                                        <input type="text" id="title_color_text" class="form-control form-control-sm"
                                            value="<?= h($title_color) ?>" maxlength="7" placeholder="#1c1e21"
                                            style="width:80px;"
                                            oninput="if(/^#[0-9a-fA-F]{6}$/.test(this.value)){document.getElementById('title_color_picker').value=this.value;}syncStyleJson();">
                                        <span id="title_color_preview" style="font-size:.78rem;font-weight:700;color:<?= h($title_color) ?>;">Aa</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row mb-2 align-items-start">
                                <div class="col-md-8">
                                    <label class="mb-0" style="font-size:.78rem;">후팝업 상세설명</label>
                                    <textarea name="pg_aft_ad_desc" id="pg_aft_ad_desc"
                                            class="form-control form-control-sm" rows="2"
                                            style="resize:vertical;"><?= htmlspecialchars($resultOne['pg_aft_ad_desc'] ?? '') ?></textarea>
                                </div>
                                <div class="col-md-4">
                                    <label class="mb-0" style="font-size:.78rem;">설명 색상</label>
                                    <div style="display:flex;align-items:center;gap:5px;">
                                        <input type="color" id="desc_color_picker" value="<?= h($desc_color) ?>"
                                            style="width:32px;height:28px;padding:0;border:1px solid #ced4da;border-radius:4px;cursor:pointer;"
                                            oninput="document.getElementById('desc_color_text').value=this.value;syncStyleJson();">
                                        <input type="text" id="desc_color_text" class="form-control form-control-sm"
                                            value="<?= h($desc_color) ?>" maxlength="7" placeholder="#65676b"
                                            style="width:80px;"
                                            oninput="if(/^#[0-9a-fA-F]{6}$/.test(this.value)){document.getElementById('desc_color_picker').value=this.value;}syncStyleJson();">
                                        <span id="desc_color_preview" style="font-size:.78rem;color:<?= h($desc_color) ?>;">Aa</span>
                                    </div>
                                    <input type="hidden" name="pg_aft_ad_style" id="pg_aft_ad_style" value="<?= h(json_encode(['titlebg' => $title_color, 'descbg' => $desc_color])) ?>">
                                </div>
                            </div>
                            ...
                            <div class="row mb-2 align-items-center">
                                <div class="col-md-12">
                                    <label class="mb-0" style="font-size:.78rem;">뱃지 <span class="text-muted" style="font-size:.7rem;">(5자 이내)</span></label>
                                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:3px;">

                                        <!-- 배경색 -->
                                        <div style="display:flex;align-items:center;gap:4px;">
                                            <span style="font-size:.72rem;color:#666;">배경</span>
                                            <input type="color" id="badge_bg_picker" value="<?= h($badge_bg) ?>"
                                                style="width:32px;height:28px;padding:0;border:1px solid #ced4da;border-radius:4px;cursor:pointer;"
                                                oninput="document.getElementById('badge_bg_text').value=this.value;syncBadgeJson();">
                                            <input type="text" id="badge_bg_text" class="form-control form-control-sm"
                                                value="<?= h($badge_bg) ?>" maxlength="7" style="width:80px;"
                                                oninput="if(/^#[0-9a-fA-F]{6}$/.test(this.value)){document.getElementById('badge_bg_picker').value=this.value;}syncBadgeJson();">
                                        </div>

                                        <!-- 이름 -->
                                        <div style="display:flex;align-items:center;gap:4px;">
                                            <span style="font-size:.72rem;color:#666;">이름</span>
                                            <input type="text" id="badge_name_text" class="form-control form-control-sm"
                                                value="<?= h($badge_name) ?>" maxlength="5" placeholder="HOT"
                                                style="width:65px;"
                                                oninput="syncBadgeJson();">
                                        </div>

                                        <!-- 텍스트색 -->
                                        <div style="display:flex;align-items:center;gap:4px;">
                                            <span style="font-size:.72rem;color:#666;">텍스트</span>
                                            <input type="color" id="badge_color_picker" value="<?= h($badge_color) ?>"
                                                style="width:32px;height:28px;padding:0;border:1px solid #ced4da;border-radius:4px;cursor:pointer;"
                                                oninput="document.getElementById('badge_color_text').value=this.value;syncBadgeJson();">
                                            <input type="text" id="badge_color_text" class="form-control form-control-sm"
                                                value="<?= h($badge_color) ?>" maxlength="7" style="width:80px;"
                                                oninput="if(/^#[0-9a-fA-F]{6}$/.test(this.value)){document.getElementById('badge_color_picker').value=this.value;}syncBadgeJson();">
                                        </div>

                                        <!-- 미리보기 -->
                                        <div style="display:flex;align-items:center;gap:4px;">
                                            <span style="font-size:.72rem;color:#666;">미리보기</span>
                                            <span id="badge_preview"
                                                style="display:inline-block;padding:2px 10px;border-radius:4px;
                                                        font-size:.72rem;font-weight:700;
                                                        background:<?= h($badge_bg) ?>;color:<?= h($badge_color) ?>;">
                                                <?= h($badge_name) ?: 'HOT' ?>
                                            </span>
                                        </div>

                                        <input type="hidden" name="pg_aft_ad_badge" id="pg_aft_ad_badge" value="<?= h(json_encode(['bg' => $badge_bg, 'name' => $badge_name, 'color' => $badge_color])) ?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>노출 시작일 &amp; 종료일</label>
                                        <div class="input-group">
                                            <input class="form-control" id="pg_aft_ad_sdate" name="pg_aft_ad_sdate"
                                                value="<?php echo $resultOne['pg_aft_ad_sdate'] ?>" readonly placeholder="시작일">
                                            <input class="form-control" id="pg_aft_ad_edate" name="pg_aft_ad_edate"
                                                value="<?php echo $resultOne['pg_aft_ad_edate'] ?>" readonly placeholder="종료일">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-md-5">
                            <div class="form-group mb-0">
                                <label>
                                    후팝업 이미지 <span class="text-danger">*</span>
                                    <small class="text-muted">(PNG, JPG · 최대 5장)</small>
                                </label>
                                <div class="custom-file mb-2">
                                    <input type="file" class="custom-file-input" id="pg_aft_ad_img"
                                        name="pg_aft_ad_img_dummy"
                                        accept="image/png, image/jpeg"
                                        multiple
                                        onchange="aft_pop_add_files(this)">
                                    <label class="custom-file-label" for="pg_aft_ad_img" id="label_aft_pop_img">
                                        <?php echo $aft_pop_files
                                            ? count($aft_pop_files) . '장 저장됨 (추가 가능)'
                                            : '이미지 파일 선택 (최대 5장)' ?>
                                    </label>
                                </div>
                                <div id="pop_img_carousel_box" class="mb-2" <?php echo empty($aft_pop_files) ? 'style="display:none"' : '' ?>>
                                    <div id="aftPopSlider" style="border:1px solid #ddd;border-radius:4px;background:#f8f9fa;position:relative;min-height:60px;">
                                        <div id="aftPopSliderInner">
                                            <?php foreach ($aft_pop_files as $fi => $f):
                                                $img_url = $f['file_path'] . '/' . $f['file_name'];
                                            ?>
                                            <div class="aslide" data-type="existing" data-file-idx="<?php echo $f['file_idx'] ?>"
                                                style="display:<?= $fi===0?'block':'none' ?>; position:relative;">
                                                <img src="<?php echo $img_url ?>"
                                                    style="max-width:100%;max-height:220px;object-fit:contain;display:block;margin:0 auto;"
                                                    class="img-thumbnail border-0">
                                                <button type="button" class="btn btn-danger btn-xs aslide-del"
                                                        style="position:absolute;top:5px;right:5px;z-index:10;">
                                                    <i class="fas fa-times"></i>
                                                </button>
                                                <input type="hidden" name="aft_pop_keep_idx[]"
                                                    id="keep_<?php echo $f['file_idx'] ?>"
                                                    value="<?php echo $f['file_idx'] ?>">
                                                <div class="text-center pb-1" style="font-size:11px;color:#999;">기존 <?php echo $fi+1 ?>장</div>
                                            </div>
                                            <?php endforeach; ?>
                                        </div>
                                        <button type="button" id="aftPopPrev" onclick="aftSlide(-1)"
                                            style="display:none;position:absolute;left:0;top:50%;transform:translateY(-50%);background:rgba(0,0,0,.3);border:none;color:#fff;width:28px;height:40px;z-index:10;">&#8249;</button>
                                        <button type="button" id="aftPopNext" onclick="aftSlide(1)"
                                            style="display:none;position:absolute;right:0;top:50%;transform:translateY(-50%);background:rgba(0,0,0,.3);border:none;color:#fff;width:28px;height:40px;z-index:10;">&#8250;</button>
                                    </div>
                                    <small class="text-muted">X 버튼으로 개별 삭제 · 총 <span id="totalImgCount"><?php echo count($aft_pop_files) ?></span>장</small>
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    $('#pageForm').on('submit', function(e) {
        ...
        for (var i = 0; i < _newFiles.length; i++) {
            formData.append('pg_aft_ad_img[]', _newFiles[i]);
        }
        ...
        $.ajax({
            ...
            error: function() {
                ...
                document.getElementById("btn_small").disabled = false;
                document.getElementById("btn_normal").disabled = false;
            }
        });
    });

    aft_pop_update_controls();

    // X버튼 이벤트 위임 — Bootstrap 캐러셀 충돌 없음
    $(document).on('click', '#aftPopSliderInner .aslide-del', function(e) {
        e.stopPropagation();
        e.preventDefault();
        aft_pop_remove_item(this);
        return false;
    });

    $('#pg_aft_ad_news_sec').on('change', toggleMainChk);
    toggleMainChk();

    var aftSdate = '<?php echo $resultOne['pg_aft_ad_sdate'] ?>';
    var aftEdate = '<?php echo $resultOne['pg_aft_ad_edate'] ?>';

    // '0000-00-00' 또는 빈값이면 무효 처리
    if (!aftSdate || aftSdate === '0000-00-00') aftSdate = '';
    if (!aftEdate || aftEdate === '0000-00-00') aftEdate = '';

    var defaultStart = aftSdate ? aftSdate : moment().format('YYYY-MM-DD');
    var defaultEnd   = aftEdate ? aftEdate : moment().add(1, 'month').format('YYYY-MM-DD');

    $('#pg_aft_ad_sdate, #pg_aft_ad_edate').daterangepicker({
        locale: {
            format: 'YYYY-MM-DD',
            applyLabel: '확인',
            cancelLabel: '취소',
        ...
        $('#pg_aft_ad_sdate').val(start.format('YYYY-MM-DD'));
        $('#pg_aft_ad_edate').val(end.format('YYYY-MM-DD'));
    });

    $('#pg_aft_ad_sdate').val(aftSdate || '');
    $('#pg_aft_ad_edate').val(aftEdate || '');
...
function validateForm() {
    ...
    // ── 후팝업 필수값 검증 ──
    if (document.getElementById('pg_aft_ad_yn').checked) {

        if (!$('#pg_aft_ad_cat').val()) {
            alert('후팝업 카테고리를 선택해주세요.');
            return false;
        }

        if (!$('#pg_aft_ad_news_sec').val()) {
            alert('노출섹션을 선택해주세요.');
            return false;
        }

        if (!$.trim($('input[name=pg_aft_ad_title]').val())) {
            alert('후팝업 타이틀을 입력해주세요.');
            return false;
        }
        if (!$('input[name=pg_aft_ad_sdate]').val()) {
            alert('노출 시작일을 입력해주세요.');
            return false;
        }
        if (!$('input[name=pg_aft_ad_edate]').val()) {
            alert('노출 종료일을 입력해주세요.');
            return false;
        }
        // 시작일 <= 종료일 체크
        if ($('input[name=pg_aft_ad_sdate]').val() > $('input[name=pg_aft_ad_edate]').val()) {
            alert('노출 종료일은 시작일 이후여야 합니다.');
            return false;
        }
        // 이미지 검증
        if (!validate_aft_pop()) return false;
    }
...
// 후팝업 스위치 토글
$('input[type=checkbox][name=pg_aft_ad_yn]').on('change', function () {
    if ($(this).is(':checked')) {
...
// 슬라이드 이동
function aftSlide(dir) {
    var slides = document.querySelectorAll('#aftPopSliderInner .aslide');
    if (slides.length === 0) return;
    slides[_slideIdx].style.display = 'none';
    _slideIdx = (_slideIdx + dir + slides.length) % slides.length;
    slides[_slideIdx].style.display = 'block';
}

// 파일 추가 (누적)
function aft_pop_add_files(input) {
    if (!input.files || !input.files.length) return;

    var existingCount = document.querySelectorAll('input[name="aft_pop_keep_idx[]"]').length;
    var addFiles = Array.from(input.files);
    var total = existingCount + _newFiles.length + addFiles.length;

    if (total > 5) {
        alert('기존 ' + existingCount + '장 + 선택중 ' + _newFiles.length + '장 + 추가 ' + addFiles.length + '장 = ' + total + '장\n최대 5장까지 가능합니다.');
        input.value = '';
        return;
    }
    ...
    addFiles.forEach(function(file) {
        _newFiles.push(file);
        aft_pop_append_new_item(file, _newFiles.length - 1);
    });

    input.value = '';
    aft_pop_update_label();
}

// 새 파일 1개를 슬라이더에 추가
function aft_pop_append_new_item(file, newIdx) {
    var inner = document.getElementById('aftPopSliderInner');
    var box   = document.getElementById('pop_img_carousel_box');

    var reader = new FileReader();
    reader.onload = function(e) {
        var slide = document.createElement('div');
        slide.className = 'aslide';
        slide.setAttribute('data-type', 'new');
        slide.setAttribute('data-new-idx', newIdx);
        ...
        slide.innerHTML =
            ...
            '</div>' +
            '<div class="text-center pb-1" style="font-size:11px;color:#0056b3;">새파일: ' + file.name + '</div>';
...
// X버튼 삭제 처리
function aft_pop_remove_item(btn) {
    var slide = btn.closest('.aslide');
    if (!slide) return;
    ...
    if (type === 'existing') {
        ...
        var fileIdx = slide.getAttribute('data-file-idx');
        var hidden = document.getElementById('keep_' + fileIdx);
        if (hidden) hidden.parentNode.removeChild(hidden);

    } else if (type === 'new') {
        var newIdx = parseInt(slide.getAttribute('data-new-idx'), 10);
        _newFiles.splice(newIdx, 1);
        document.querySelectorAll('#aftPopSliderInner .aslide[data-type="new"]').forEach(function(el) {
            var idx = parseInt(el.getAttribute('data-new-idx'), 10);
            if (idx > newIdx) el.setAttribute('data-new-idx', idx - 1);
        });
    }
...
// 슬라이더 상태 갱신
function aft_pop_update_controls() {
    var slides  = document.querySelectorAll('#aftPopSliderInner .aslide');
    var total   = slides.length;
    var box     = document.getElementById('pop_img_carousel_box');
    var countEl = document.getElementById('totalImgCount');
    ...
    slides.forEach(function(s, i) {
        s.style.display = i === _slideIdx ? 'block' : 'none';
    });

    var prev = document.getElementById('aftPopPrev');
    var next = document.getElementById('aftPopNext');
    if (prev) prev.style.display = total > 1 ? 'block' : 'none';
    if (next) next.style.display = total > 1 ? 'block' : 'none';
}

// 라벨 업데이트
function aft_pop_update_label() {
    var existingCount = document.querySelectorAll('input[name="aft_pop_keep_idx[]"]').length;
    var newCount      = _newFiles.length;
    var total         = existingCount + newCount;
    var label         = document.getElementById('label_aft_pop_img');
    if (!label) return;
    if (total === 0) {
        label.textContent = '이미지 파일 선택 (최대 5장)';
    } else {
        var parts = [];
        if (existingCount > 0) parts.push('기존 ' + existingCount + '장');
        if (newCount > 0)      parts.push('새 ' + newCount + '장');
        label.textContent = parts.join(' + ') + ' = 총 ' + total + '장 (5장 가능)';
    }
}

// 노출섹션 변경시 대표배너 체크박스 활성/비활성
function toggleMainChk() {
    var sec = $('#pg_aft_ad_news_sec').val();
    var chk = $('#pg_aft_ad_is_main');
    if (sec === 'normal' || sec === '') {
        chk.prop('checked', false).prop('disabled', true);
        chk.closest('.form-group').find('label.ml-1').css('color','#aaa');
    } else {
        chk.prop('disabled', false);
        chk.closest('.form-group').find('label.ml-1').css('color','');
    }
}

function syncBadgeJson() {
    var bg    = document.getElementById('badge_bg_text').value    || '#ef4444';
    var name  = document.getElementById('badge_name_text').value  || '';
    var color = document.getElementById('badge_color_text').value || '#ffffff';
    document.getElementById('pg_aft_ad_badge').value = JSON.stringify({bg:bg,name:name,color:color});
    var prev = document.getElementById('badge_preview');
    prev.style.background = bg;
    prev.style.color      = color;
    prev.textContent      = name || 'HOT';
}

function syncStyleJson() {
    var titlebg = document.getElementById('title_color_text').value || '#1c1e21';
    var descbg  = document.getElementById('desc_color_text').value  || '#65676b';
    document.getElementById('pg_aft_ad_style').value = JSON.stringify({titlebg:titlebg,descbg:descbg});
    document.getElementById('title_color_preview').style.color = titlebg;
    document.getElementById('desc_color_preview').style.color  = descbg;
}


// 이미지 유효성 검사
function validate_aft_pop() {
    if (!document.getElementById('pg_aft_ad_yn').checked) return true;
    var keepCount = document.querySelectorAll('input[name="aft_pop_keep_idx[]"]').length;
    if (keepCount === 0 && _newFiles.length === 0) {
