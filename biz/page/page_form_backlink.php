...
<section class="content">
    <div class="container-fluid">
        <form name="pageForm" id="pageForm" action="./page_form_update" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">백링크설정</h3>
                    
                    <label class="ml-4 switch">
                        <input type="checkbox" id="pg_backlink_yn" name="pg_backlink_yn" class="switch-input" <?php echo $resultOne['pg_backlink_yn'] == "Y" ? "checked" : ""?>>
                        <span class="switch-label" data-on="ON" data-off="OFF"></span>
                        <span class="switch-handle"></span>
                    </label>

                    <div id="rdo_grp1" name="rdo_grp" class="ml-4 custom-control custom-radio custom-control-inline <?php echo $resultOne['pg_backlink_yn'] != "Y" ? "d-none" : ""?>">
                        <div class="custom-file">
                            <input type="file" name="des_shortcut" id="des_shortcut" accept="image/*">
                            
                            <!-- 
                            1. load시 이미지가 있으면 툴팁으로 이미지 보여주게 할것
                            2. load시 이미지가 없으면 front에서 이미지만 업로드 가능하게 할것 
                            -->
                        </div>
                    </div>
                ...
                <div id="divApiSet" class="card-body <?php echo $resultOne['pg_api_kind'] != "normal" ? "d-none" : ""?>">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>타이틀</label>
                                <input type="text" id=pg_backlink_title name="pg_backlink_title" class="form-control" value="<?php echo $resultOne['pg_backlink_title'] ?>" >    
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>기간</label>
                                <select id="pg_backlink_term" name="pg_backlink_term" class="form-control">
                                    <option value="1" <?php echo get_selected($resultOne['pg_backlink_term'], '1'); ?>> 1. 영구</option>
                                    <option value="2" <?php echo get_selected($resultOne['pg_backlink_term'], '2'); ?>> 2. 1주일</option>
                                    <option value="3" <?php echo get_selected($resultOne['pg_backlink_term'], '3'); ?>> 3. 1개월</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>정렬</label>
                                <select id="pg_backlink_pos" name="pg_backlink_pos" class="form-control">
                                    <option value="1" <?php echo get_selected($resultOne['pg_backlink_pos'], '1'); ?>> 1. 상단고정</option>
                                    <option value="2" <?php echo get_selected($resultOne['pg_backlink_pos'], '2'); ?>> 2. 랜덤</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="selectpicker form-control" id="pg_api_add_param" name="pg_api_add_param[]" multiple>
                                    ...
                                    ?>
                                        <option value="<?php echo $scr_arr[$j] ?>" class="bg-warning" selected><?php echo $scr_arr[$j] ?></option>
                                    <?php  
                                        } 
                                    }
                                    ?>
...
$(document).ready(function() {
    ...
    $(document).on('click', '#remove-img', function() {
        ...
        $('input[name="existing_image"]').val(''); // 기존 이미지 경로 초기화
    });
