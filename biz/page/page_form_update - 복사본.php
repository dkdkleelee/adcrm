...
// URL 도메인 형식 유효성 검사
if (!empty($pg_redirect_url) && !preg_match("~^(?:f|ht)tps?://~i", $pg_redirect_url)) {
    // http://로 시작하는 경우에만 변경하지 않음
    if (!preg_match("~^http://~i", $pg_redirect_url)) {
...
// 후팝업 관련 변수 할당
$pg_aft_ad_yn     = isset($_POST['pg_aft_ad_yn']) ? 'Y' : 'N';
$pg_aft_ad_cat    = isset($_POST['pg_aft_ad_cat']) ? strip_tags(clean_xss_attributes($_POST['pg_aft_ad_cat'])) : '';
$pg_aft_ad_size   = isset($_POST['pg_aft_ad_size']) ? (int)$_POST['pg_aft_ad_size'] : 0;
...
$pg_aft_ad_file_idx = isset($_POST['pg_aft_ad_file_idx']) ? (int)$_POST['pg_aft_ad_file_idx'] : 0;
...
if($pg_api_yn == "on") {
    ...
    if($pg_api_kind == "normal") {
        ...
        if(!empty($pg_api_add_param)) {
            $api_add_param = implode( ',', $pg_api_add_param );
            $api_add_param = str_replace(",", "&", $api_add_param);
            }
...
if ($pg_aft_ad_yn == 'Y') {
    // 1. 경로 설정 (data/file/aftpop/$page_idx)
    $aft_pop_base = G5_DATA_PATH.'/file/aftpop';
    $aft_pop_path = $aft_pop_base.'/'.$page_idx;

    // 공통 폴더 및 페이지별 전용 폴더 생성
    if(!is_dir($aft_pop_base)) { @mkdir($aft_pop_base, G5_DIR_PERMISSION); @chmod($aft_pop_base, G5_DIR_PERMISSION); }
    if(!is_dir($aft_pop_path)) { @mkdir($aft_pop_path, G5_DIR_PERMISSION); @chmod($aft_pop_path, G5_DIR_PERMISSION); }

    // 2. 기존 이미지 삭제 (삭제 체크 시)
    if (isset($_POST['del_aft_pop_img']) && $_POST['del_aft_pop_img'] == '1' && $pg_aft_ad_file_idx) {
        $file_row = sql_fetch(" SELECT file_name FROM gnp_crm_aft_pop_file WHERE file_idx = '$pg_aft_ad_file_idx' ");
        if ($file_row['file_name']) {
            @unlink($aft_pop_path . '/' . $file_row['file_name']);
        }
        sql_query(" DELETE FROM gnp_crm_aft_pop_file WHERE file_idx = '$pg_aft_ad_file_idx' ");
        $pg_aft_ad_file_idx = 0;
    }

    // 3. 신규 이미지 업로드
    if ($_FILES['pg_aft_ad_img']['name']) {
        // 교체를 위해 기존 파일 정보가 있다면 삭제
        if ($pg_aft_ad_file_idx) {
            $file_row = sql_fetch(" SELECT file_name FROM gnp_crm_aft_pop_file WHERE file_idx = '$pg_aft_ad_file_idx' ");
            if ($file_row['file_name']) { @unlink($aft_pop_path . '/' . $file_row['file_name']); }
            sql_query(" DELETE FROM gnp_crm_aft_pop_file WHERE file_idx = '$pg_aft_ad_file_idx' ");
        }

        $filename = $_FILES['pg_aft_ad_img']['name'];
        $tname = $_FILES['pg_aft_ad_img']['tmp_name'];
        
        // 파일명 난수화 (그누보드 스타일)
        $file_ext = pathinfo($filename, PATHINFO_EXTENSION);
        $new_filename = 'aftpop_' . date('YmdHis') . '_' . substr(md5(microtime()), 0, 6) . '.' . $file_ext;

        if (move_uploaded_file($tname, $aft_pop_path . '/' . $new_filename)) {
            $file_source = $filename;
            $file_size = $_FILES['pg_aft_ad_img']['size'];
            
            $file_sql = " INSERT INTO gnp_crm_aft_pop_file 
                            SET page_idx = '$page_idx',
                                file_source = '$file_source',
                                file_name = '$new_filename',
                                file_path = '/data/file/aftpop/{$page_idx}',
                                file_size = '$file_size',
                                insert_date = NOW() ";
            sql_query($file_sql);
            $pg_aft_ad_file_idx = sql_insert_id();
        }
    }
}
...
sql_query(" SET autocommit=0 ");
 ...
 if ($w == '') {
    ...
    update {$g5['crm_page']} set
         ...
         , pg_aft_ad_size    = '{$pg_aft_ad_size}'
         , pg_aft_ad_title   = '{$pg_aft_ad_title}'
         , pg_aft_ad_desc    = '{$pg_aft_ad_desc}'
         , pg_aft_ad_sdate   = '{$pg_aft_ad_sdate}'
         , pg_aft_ad_edate   = '{$pg_aft_ad_edate}'
