...
if ($w == '') {
  ...
  ";
    ...
    $ptn_ban_phones = $resultOne['ptn_ban_phone'];
    $ptn_ban_phones = [];
    if (!empty($resultOne['ptn_ban_phone'])) {
        $ptn_ban_phones = explode(',', $resultOne['ptn_ban_phone']);
    }
...
$disabled = ($member['mb_level'] <= 5) ? 'disabled' : '';
?>
...
<section class="content">
    <div class="container-fluid">
        <form name="listForm" id="listForm" action="./partner_form_update" method="post" enctype="multipart/form-data" onSubmit="return validateForm()">
            ...
            <div class="card card-danger">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-1">
                            <div class="form-group">
                                <label><code>코드*(숫자만)</code></label>
                                <input type="text" class="form-control" id="option-value" placeholder="코드(ex:1~9)">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><code>값*(사용자한테 보이는값)</code></label>
                                <input type="text" class="form-control" id="option-label" placeholder="값(예:리콜성공)">
                            </div>
                        </div>
                        <div class="col-md-1">
                            <div class="form-group">
                                <label><code>추가*</code></label>
                                <button type="button" class="btn btn-primary w-100" id="add-option-btn">추가</button>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><code>고객사admin용 상태값*</code></label>
                                <select class="form-control" id="user-select" name="ptn_status_custom[]">
                                    <?php
                                    if (empty($w)) {
                                        // 신규 입력일 경우 ptn_status_custom을 "basic"으로 설정
                                        $ptn_status_custom = "basic";
                                    } else {
                                        // 기존 입력일 경우 기존 값을 사용
                                        $ptn_status_custom = $resultOne['ptn_status_custom'];
                                    }

                                    if ($ptn_status_custom == "basic") {
                                        echo '<option value="basic">해당 데이터 지우지 않으면 기본값으로 적용됨</option>';
                                        echo '<option value="1">1:부재</option>';
                                        echo '<option value="2">2:불량</option>';
                                        echo '<option value="3">3:거절</option>';
                                        echo '<option value="4">4:리콜</option>';
                                        echo '<option value="5">5:중복</option>';
                                        echo '<option value="6">6:유망</option>';
                                        echo '<option value="7">7:승인</option>';
                                    } else {
                                        // 커스텀 옵션
                                        $customOptions = explode('||', $resultOne['ptn_status_custom']);
                        ...
                        <div class="col-md-1">
                            <div class="form-group">
                                <label>삭제</label>
                                <button type="button" class="btn btn-danger w-100" id="delete-option-btn">삭제</button>
                            </div>
                        </div>
                        <div class="col-md-1">
                            <div class="form-group">
                                <label>초기화</label>
                                <button type="button" class="btn btn-info w-100" id="init-btn">초기화</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- 여기에 div master 1,2,3 추가 ? 아니면 script로 추가할지 파악 -->
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">고객사 어드민 툴팁적용</h3>
                    <label class="ml-4 switch">
                        <input type="checkbox" id="ptn_is_tooltip" name="ptn_is_tooltip" class="switch-input" <?php echo $resultOne['ptn_is_tooltip'] == "Y" ? "checked" : "" ?>>
                        <span class="switch-label" data-on="ON" data-off="OFF"></span>
                        <span class="switch-handle"></span>
                    </label>
                </div>
            ...
            <div class="card-header">
                <div class="d-flex align-items-center">
                    ...
                    <!-- <div class="ml-2 d-flex align-items-center">
                        ...
                        <input type="checkbox" id="historyCheckbox" name="historyCheckbox" style="height: auto; width: auto; margin-left: 8px;" <?php echo $w != "u" ? " disabled" : "" ?>>)
                    </div> -->

                    <input type="hidden" id="historyCheckbox" name="historyCheckbox" value="off">

                    <div class="ml-2 d-flex align-items-center">
                        <label for="historyCheckbox" class="mb-0">(dashboard</label>
                        <input type="checkbox" id="ptn_show_dash" name="ptn_show_dash" style="height: auto; width: auto; margin-left: 8px;" <?php echo $resultOne['ptn_show_dash'] == 'Y' ? 'checked' : ''; ?>>)
                    </div>

                </div>
            </div>

            <div class="card-body" id="contractCardBody">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label id="lb_cont_hist">(참고)계약단가</label>
                            <input type="text" name="ptn_cont_ref" id="ptn_cont_ref" class="form-control" value="<?php echo $resultOne['ptn_cont_ref'] ?>">
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>AS수량(양수입력)</label>
                            <input type="number" name="ptn_as_amt" id="ptn_as_amt" class="form-control" value="<?php echo $resultOne['ptn_as_amt'] ?>" min="0">
                        </div>
                    </div>
                </div>
                ...
                <?php $isCpaOrNull = $resultOne['ptn_ad_gubun'] == "CPA" || $resultOne['ptn_ad_gubun'] === null; ?>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <div class="input-group-text">
                                        <input type="checkbox" id="ptn_ntc_useyn" name="ptn_ntc_useyn" <?php echo $isCpaOrNull ? 'disabled' : ''; ?>>
                                    </div>
                                </div>
                                <input type="date" id="ptn_ntc_date" name="ptn_ntc_date" class="form-control" value="<?php echo $ptn_ntc_date; ?>" <?php echo $isCpaOrNull ? 'disabled' : ''; ?>>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                            ...
                            <tbody>
                                <?php for ($i = 0; $hist_row = sql_fetch_array($hist_list); $i++) {
                                    ...
                                    $current_db_cnt = sql_fetch($current_db_cnt_sql);
                                    $term_cnt = $current_db_cnt['term_cnt'];
                                    $achievement_rate = $hist_row['ptn_db_amount'] > 0 ? (($term_cnt - $hist_row['ptn_as_amt']) / $hist_row['ptn_db_amount']) * 100 : 0; // 달성률 계산
                                ?>
                                    <tr>
                                        <td><?php echo $i + 1 ?></td>
                                        ...
                                        <td><?php echo number_format($hist_row['ptn_as_amt']) ?>건</td>
                                        ...
                                        <td><button type='button' class='btn btn-danger btn-xs' onclick='delete_ptn_hist(this, "<?php echo $hist_row['hist_idx'] ?>")' <?php echo $disabled; ?>>삭제</button></td>
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label>고객센터</label>
                        <input type="text" id="ptn_tel" name="ptn_tel" class="form-control" value="<?php echo $resultOne['ptn_tel'] ?>" placeholder="고객센터(02-1234-5678)" oninput="telHyphen(this);" maxlength="13">
                    </div>
                </div>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control" id="ptn_ban_phone" name="ptn_ban_phone">
                                ...
                                <?php foreach ($ptn_ban_phones as $tel):
                                    $trimmedPhone = htmlspecialchars(trim($tel));
                                ?>
                                    <option value="<?php echo $trimmedPhone; ?>">
                                        <?php echo $trimmedPhone; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <button type="button" id="deleteOption" class="btn btn-danger">삭제</button>
                            <input type="hidden" id="selectedPhones" name="ptn_ban_phone_array">
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        // 1-1. 코드값 selecte 사용자지정 modify 추가버튼
        $('#add-option-btn').on('click', function() {
            let value = $('#option-value').val().trim();
            let label = $('#option-label').val().trim();

            $('#option-value').removeClass('is-invalid');
            $('#option-label').removeClass('is-invalid');

            if (value === "" || label === "") {
                alert("값과 라벨을 모두 입력해주세요.");
                if (value === "") {
                    $('#option-value').addClass('is-invalid');
                }
                if (label === "") {
                    $('#option-label').addClass('is-invalid');
                }
                return;
            }

            if (!/^\d+$/.test(value)) {
                alert("값은 숫자만 입력할 수 있습니다.");
                $('#option-value').addClass('is-invalid');
                return;
            }

            if (value.length !== 1) {
                alert("값은 숫자 1글자이어야 합니다.");
                $('#listForm')[0].reset(); // 올바른 폼 ID로 변경
                return;
            }


            if (label.length > 20) {
                alert("라벨은 20자 이내로 입력해주세요.");
                $('#option-label').addClass('is-invalid');
                return;
            }
            ...
            $('#user-select').append(
                $('<option></option>').val(value).text(value + ": " + label)
            );
            $('#admin-form')[0].reset();
        });

        // 1-2. 코드값 selecte 사용자지정 modify 삭제버튼
        $('#delete-option-btn').on('click', function() {
            let selectedValue = $('#user-select').val();
            let confirmDelete = confirm(`정말로 값 "${selectedValue}"을(를) 삭제하시겠습니까?`);
        ...
        // 1-3. 코드값 selecte 사용자지정 modify 초기화버튼
        $('#init-btn').on('click', function() {
            var defaultOptions = `
            <option value="basic">해당 데이터 지우지 않으면 기본값으로 적용됨</option>
            <option value="1">1:부재</option>
            <option value="2">2:불량</option>
            <option value="3">3:거절</option>
            <option value="4">4:리콜</option>
            <option value="5">5:중복</option>
            <option value="6">6:유망</option>
            <option value="7">7:승인</option>`;

            $('#user-select').html(defaultOptions);
            $('#listForm')[0].reset();
            $('#option-value').removeClass('is-invalid');
            $('#option-label').removeClass('is-invalid');
            alert("옵션이 초기화되었습니다.");
        ...
        // 2-1.연락처 차단 로직 추가버튼
        var selectedPhones = <?php echo json_encode($ptn_ban_phones); ?> || [];
        $('#addOption').click(function() {
            ...
            // 이미 존재하는 값인지 확인
            if (!selectedPhones.includes(newOption)) {
                selectedPhones.push(newOption); // 배열에 추가
                $('#ptn_ban_phone').append(`<option value="${newOption}">${newOption}</option>`); // select에 추가
            }

            $('#newOption').val(''); // 텍스트 필드 초기화
            updateHiddenField(); // 숨겨진 필드 업데이트
        });
        ...
        $('#deleteOption').click(function() {
            ...
            if (selectedOption.val() && selectedOption.val() !== '') {
                var valueToRemove = selectedOption.val();
                selectedPhones = selectedPhones.filter(function(tel) {
                    return tel !== valueToRemove;
                });
                selectedOption.remove(); // select에서 제거
                updateHiddenField(); // 숨겨진 필드 업데이트
            } else {
                alert('삭제할 옵션을 선택하세요.');
            }
        ...
        // 2-4.연락처 차단 로직 -> 배열을 쉼표로 구분된 문자열로 저장
        function updateHiddenField() {
            $('#selectedPhones').val(selectedPhones.join(',')); 
        }

        // 3-1. History 이벤트 
        var alertFlag = false;
        var $adGubun = $('#ptn_ad_gubun');
        var $label = $('#lb_cont_hist');
        var $input = $('#ptn_cont_ref');
        var originalValues = {
            CPA: '',
            CPC: '',
            CPP: ''
        };
        var initialSelected = $adGubun.val();
        originalValues[initialSelected] = $input.val();
        var inputModified = false; // 사용자 입력이 변경되었는지 확인하는 변수

        // 3-2. History 이벤트 감지시 기록하기 체크박스 활성화 
        $('#contractCardBody').on('change', 'input, select, textarea', function(event) {
            if (event.originalEvent !== undefined) {
                if (alertFlag == false) {
                    $('#historyCheckbox').val('on');
                    alertFlag = true;
                }
            }
        });

        // 3-2. History 이벤트 구분 selected 변경시 이벤트
        $adGubun.change(function() {
            var selected = $(this).val();
            var previous = $adGubun.data('previous');

            if (previous) {
                var currentValue = $input.val();
                originalValues[previous] = currentValue;
            }

            if (selected === 'CPA') {
                $label.text('(참고)계약단가');
                var value = originalValues[selected] || '';
            ...
            $adGubun.data('previous', selected);
            inputModified = false;
        });
        ...
        $adGubun.data('previous', initialSelected);
        $adGubun.change();
        ...
        // 4-2. DB분배 설정 -> 부모페이지 변경시 이벤트
        $(document).on('change', '[id^=share_child_ptn]', function() {
    ...
    function init_db_share(ptn_idx) {
        ...
        if (result) {
            ...
            $.ajax({
                ...
                data: {
                    ptn_idx: ptn_idx,
                    hidden_page_idx: hidden_page_idx,
                    act: "init_db_share"
                },
                url: "partner_ajax",
                dataType: "json",
    ...
    function validateForm() {
        var form = $('#listForm');
        form.find('input[name="ptn_status_custom"]').remove();
        var hasBasicOption = $('#user-select option[value="basic"]').length > 0;

        if (hasBasicOption) {
            // "basic" 옵션이 존재하면 ptn_status_custom을 "basic"으로 설정
            var hiddenInput = $('<input>').attr({
                type: 'hidden',
                name: 'ptn_status_custom',
                value: 'basic' // "basic"으로 설정
            });
            form.append(hiddenInput);
        } else {
            var ptn_status_custom_arr = [];

            $('#user-select option').each(function() {
                var value = $(this).val().trim();
                var fullText = $(this).text().trim();
                var parts = fullText.split(":");
                if (value !== "" && value !== "basic" && parts.length > 1) {
                    var label = parts.slice(1).join(":").trim();
                    ptn_status_custom_arr.push(value + ":" + label);
                }
            });

            var ptn_status_custom_str = ptn_status_custom_arr.join("||");
            var hiddenInput = $('<input>').attr({
                type: 'hidden',
                name: 'ptn_status_custom',
                value: ptn_status_custom_str
            });
            form.append(hiddenInput);
        }
        ...
        if ($('#db_is_ptn_share').is(':checked')) {
            ...
            $('#divApiSet .row').each(function() {
                ...
                if (!dbPtnShare || !shareCode || !shareCnt) {
                    isValid = false;
                    message = "DB 분배 필드를 채워주세요.";
                    return false; // 루프 중단
                }

                var combination = dbPtnShare + '-' + shareCode;
                if (combinations.has(combination)) {
                    message = "중복된 고객사와 코드 조합이 있습니다.";
                    isValid = false;
                    return false; // 루프 중단
                }
