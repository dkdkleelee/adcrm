          ...
          <div class="card-header">
              <h3 class="card-title">DB분배</h3>
                ...
                <div class="custom-control custom-radio custom-control-inline">
                  ...
                  <button type="button" id="add_div" class="btn btn-warning btn-sm d-none" style="white-space: nowrap;">추가</button>
                </div>
...
<script>
  $(document).ready(function() {
    ...
    // // 페이지 로딩 시 #divApiSet의 초기 HTML 상태를 문자열로 저장합니다.
    // var initialDivApiSetHTML = $('#divApiSet').html();

    // $('#add_div').click(function() {
    //     // 새로운 .card 요소를 생성합니다.
    //     var $newCard = $('<div class="card"><div class="card-header"><h3 class="card-title">DB분배</h3></div><div class="card-body"></div></div>');
        
    //     // 저장된 초기 HTML을 새 .card의 body에 삽입합니다.
    //     $newCard.find('.card-body').html(initialDivApiSetHTML);

    //     // 페이지의 마지막 .card 뒤에 새 .card를 추가합니다.
    //     $('.card').last().after($newCard);

    //     // 새로 추가된 .card 내의 모든 selectpicker를 초기화합니다.
    ...
    // $(document).on('change', '.share_child_ptn', function() {
    ...
    //         dataType: "json",
    //         success: function(result) {
    //             // $target.empty().append(result).selectpicker('refresh');
    ...
    // $(document).off('change', '.share_child_ptn').on('change', '.share_child_ptn', function() {
    //     var ptn_idx = $(this).val();
    //     var $target = $(this).closest('.row').find('select[name^="share_child_page_idx"]');
