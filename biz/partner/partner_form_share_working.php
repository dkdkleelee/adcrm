...
<script>
  $(document).ready(function() {
    ...
    var initialFormValues = $('#contractCardBody').find('input, select, textarea').serialize();
    var alertFlag = false;
    ...
    // Initialize originalValues based on the selected ad type
    var originalValues = {
      CPA: '',
      CPC: '',
      CPP: ''
    ...
    $('#contractCardBody').on('change', 'input, select, textarea', function(event) {
      // Check if the event was triggered by the user
    ...
    $adGubun.change(function() {
      ...
      // Save the current input value before switching
      if (previous) {
      ...
      if (selected === 'CPA') {
        ...
        // Attach input event to format the value and prevent non-numeric input
        ...
        if (selected === 'CPC') {
          ...
          $label.text('(참고)계약단가'); // 기본값으로 CPA 라벨 설정
        }

        // Remove input restrictions and clean the value
        $input.off('input');
        var value = originalValues[selected] || '';
        $input.val(value);
      }

      // Save the selected value as the previous state
      $adGubun.data('previous', selected);
      inputModified = false; // Reset the modification flag
    });
    ...
    function updateHiddenField() {
      $('#selectedPhones').val(selectedPhones.join(',')); // 배열을 쉼표로 구분된 문자열로 저장
    }
