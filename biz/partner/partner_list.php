                                            ...
                                            <?php for ($i = 0; $row = sql_fetch_array($result); $i++) { 
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <?php if ($row['page_cnt'] >= 1): ?>
                                                            <i class="fas fa-list" style="cursor: pointer;" onclick="show_all_page('<?php echo $row['ptn_idx']; ?>', '<?php echo $row['ptn_nm']; ?>');"></i>
                                                            (<?php echo $row['page_cnt']; ?>)
                                                        <?php endif; ?>
...
<div class="modal fade" id="pageListModal" tabindex="-1" role="dialog" aria-labelledby="pageListModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title" id="pageListModalLabel"><i class="fas fa-plane-departure"></i> 페이지 LIST</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="pageListContent">
            <div id="pageListPagination"></div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    $(function() {
        ...
        var table = $('#tbl_partner').DataTable({
            ...
            columnDefs: [
                ...
                {responsivePriority: 104, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 5 : 3; ?>},
                {responsivePriority: 105, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 6 : 4; ?>},
                {responsivePriority: 106, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 7 : 5; ?>},
                {responsivePriority: 107, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 8 : 6; ?>},
                {responsivePriority: 107, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 9 : 7; ?>},
                {responsivePriority: 108, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 10 : 8; ?>},
                {responsivePriority: 109, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 11 : 9; ?>},
                {responsivePriority: 110, targets: <?php echo ($sfl == "deptno" && $stx == $member['mb_deptno']) ? 12 : 10; ?>, "width": "1%"}
            ]
    ...
    function show_all_page(ptn_idx, ptn_nm) {
        
        $('#pageListModalLabel').html('<i class="fas fa-plane-departure"></i> [' + ptn_nm + '] 페이지 LIST');
        //$('#pageListModal').data('ptn_idx', ptn_idx);

        var act = "show_all_page";
        var ptn_idx = ptn_idx;
        ...
        $.ajax({
            ...
            success:function(result) {
              ...
              if (result.html) {
                  ...
                  $('#pageListContent').html(result.html);
