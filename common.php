...
ini_set("session.gc_maxlifetime", 7200); // session data의 garbage collection 존재 기간을 지정 (초)
...
if (isset($_REQUEST['stx']) && $_REQUEST['stx'] == 'advanced') {
    $qstr = 'stx=advanced';
...
if (isset($_SESSION['ss_mb_id']) && $_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if( ($member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) 
        ...
        if (substr($member['mb_today_login'], 0, 10) != G5_TIME_YMD) {
            ...
            $sql = " update {$g5['member_table']} set mb_today_login = '".G5_TIME_YMDHIS."', mb_login_ip = '".getRealClientIp()."' where mb_id = '{$member['mb_id']}' ";
            sql_query($sql);
        }
    }
} else {
...
if ($bo_table) {
    ...
    if (isset($board['bo_table']) && $board['bo_table']) {
        ...
        if (isset($wr_id) && $wr_id) {
            ...
            $write = get_content_by_field($write_table, 'main', 'wr_seo_title', generate_seo_title($wr_seo_title));
