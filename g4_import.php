...
<div id="g4_import">
    <p>
        이 프로그램은 위드어스 ADMIN 설치 후 바로 실행하셔야만 합니다.<br>
        만약 위드어스 ADMIN 사이트를 운영 중에 이 프로그램을 실행하시면 DB 데이터가 망실되거나 데이터의 오류가 발생할 수 있습니다.<br>
        또한 중복해서 실행하실 경우에도 DB 데이터의 오류가 발생할 수 있으니 반드시 한번만 실행해 주십시오.
    </p>
    <p>프로그램을 실행하시려면 WithUs의 config.php 파일 경로를 입력하신 후 확인을 클릭해 주십시오.</p>
...
function fimport_submit(f)
{
    ...
    return confirm('WithUs의 DB 데이터를 이전하시겠습니까?');
}
</script>
