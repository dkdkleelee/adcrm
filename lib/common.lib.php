...
// 한페이지에 보여줄 행, 현재페이지, 총페이지수, URL
function get_paging_forBootst($write_pages, $cur_page, $total_page, $url, $add="")
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#(&amp;)?page=[0-9]*#', '', $url);
	$url .= substr($url, -1) === '?' ? 'page=' : '&amp;page=';
...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    $g5_object->set('main', $write_row['wr_id'], $write_row, $board['bo_table']);
    ...
    if( ! (isset($list['wr_seo_title']) && $list['wr_seo_title']) && $list['wr_id'] ){
        seo_title_update(get_write_table_name($board['bo_table']), $list['wr_id'], 'main');
    }
...
function get_write($write_table, $wr_id, $is_cache=false)
{
    ...
    $write = $g5_object->get('main', $wr_id, $wr_bo_table);

    if( !$write || $is_cache == false ){
        $sql = " select * from {$write_table} where wr_id = '{$wr_id}' ";
        $write = sql_fetch($sql);

        $g5_object->set('main', $wr_id, $write, $wr_bo_table);
    }
...
function get_member($mb_id, $fields='*', $is_cache=false)
{
    ...
    $sql = " select $fields ,(select sub.parent_deptno from {$g5['crm_depart']} sub where deptno = mb_deptno) as parent_deptno from {$g5['member_table']} where mb_id = TRIM('$mb_id') and mb_gubun = 'E'";
...
*/
// paging
function get_paging_bootst($write_pages, $cur_page, $total_page, $url, $add="")
{
    $url = preg_replace('#(&amp;)?page=[0-9]*#', '', $url);
	$url .= substr($url, -1) === '?' ? 'page=' : '&amp;page=';
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<li class="page-item"><a href="'.$url.'1'.$add.'" class="page-link"><i class="fas fa-angle-double-left"></i></a></li>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) {
        $str .= '<li class="page-item"><a "'.$url.($start_page-1).$add.'" class="page-link"><i class="fas fa-angle-left"></i></a></li>'.PHP_EOL;
    }
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k) {
                ...
                $str .= '<li class="page-item active"><a href="'.$url.$k.$add.'" class="page-link">'.$k.'</a></li>'.PHP_EOL;
            }
        }
    }

    if ($total_page > $end_page) {
        $str .= '<li class="page-item"><a href="'.$url.($end_page+1).$add.'" class="page-link"><i class="fas fa-angle-right"></i></a></li>'.PHP_EOL;
    }

    if ($cur_page < $total_page) {
        $str .= '<li class="page-item"><a href="'.$url.$total_page.$add.'" class="page-link"><i class="fas fa-angle-double-right"></i></a></li>'.PHP_EOL;
    }
...
function get_paging_advanced($write_pages, $cur_page, $total_page, $url, $advanced_conditions)
{
    $url = preg_replace('#\?.*#', '', $url) . '?stx=advanced'; // '?'로 시작하고, stx=advanced 추가
    $url .= '&amp;page=';

    // 고급 검색 조건을 쿼리 문자열로 변환
    $advanced_query = http_build_query($advanced_conditions, '', '&amp;');
    $add = '&' . $advanced_query;
    ...
    if ($start_page > 1) {
        $str .= '<li class="page-item"><a href="'.$url.($start_page-1).$add.'" class="page-link"><i class="fas fa-angle-left"></i></a></li>'.PHP_EOL;
    }
...
// sort
function get_sort_bootst($col, $query_string='', $flag='asc', $sst2, $sod2, $column_name)

{
    ...
    if($col == $sst2) {
        if($sod2 == "asc") {
            $order_icon = "fas fa-sort-up";
        } else {
            $order_icon = "fas fa-sort-down";
        }
        
    }else{
        $order_icon = "fas fa-sort";
    }
    ...
    //return "<a href=\"{$_SERVER['SCRIPT_NAME']}?{$qstr}\"></a>";
    return "<a href=\"{$_SERVER['SCRIPT_NAME']}?{$qstr}\">{$column_name} <i class='{$order_icon}'></i></a>";
}


function get_sort_custom($col, $query_string='', $flag='asc', $sst2, $sod2, $column_name, $stx_json)

{
    ...
    // JSON을 디코딩하여 배열로 변환
    $stx_data = json_decode($stx_json, true);

    $arr_query = array();
    $arr_query[] = $query_string;
    // JSON에서 추출된 각 값들을 쿼리 스트링에 추가
    foreach ($stx_data as $key => $value) {
        $arr_query[] = $key . '=' . urlencode($value);
    }
    $arr_query[] = $q1;
    $arr_query[] = $q2;
    $arr_query[] = 'sfl='.$sfl;
...
function sql_result_alert($result, $msg, $redirect){

    if($result == true) {
        alert($msg, $redirect);
    } else {
        alert("통신에러 관리자에게 문의해주세요.", G5_URL);
    }
}


// 회원권한을 SELECT 형식으로 얻음
function comm_select_mb_level($name, $start_id = 0, $end_id = 10, $selected = "", $event = "")
{
    global $g5;
...
function comm_select_mb_deptno($id, $name, $selected='', $event='')
{
    global $g5;
    $sql = "select *
    from {$g5['crm_depart']}
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if ($i == 0) $str .= "<option value=\"\">부서없음</option>";
        $str .= option_selected($row['deptno'], $selected, $row['deptnm']);
    }
    $str .= "</select>";
    return $str;
}

//날짜 하루전거면 MM일 HH시 당일이면 등록시
function view_dateformat($date)
{
    if($date == null || $date == "") {
        return false;
    }
    ...
    $param = date("Y-m-d", strtotime($date));
    $today = date("Y-m-d");

    if($param >= $today) {
        $str = date("m월d일 H:i분", strtotime($date)). ' <img src="'.G5_THEME_URL.'/img/new.gif" alt="새글">' ;
    } else {
        $str = date("y년m월d일 H시", strtotime($date));
    }
...
function isSqlError($result, $sql){
    if($result == false) {
        sql_query("ROLLBACK");
        error_log(date("Y-m-d h:i:s").' : '.$sql, 3, G5_DATA_PATH."/log/isSqlError.log");
        alert("에러발생 개발담당자에게 문의 이동기-010-3535-1555");
    }
...
function getRealClientIp() {
    ...
    if (getenv('HTTP_CF_CONNECTING_IP')) {
        ...
        $ipaddress = inet_ntop(substr(inet_pton($ipaddress), -4));
...
function sql_affected_rows($link=null)
{
    ...
    if(function_exists('mysqli_affected_rows') && G5_MYSQLI_USE) {
        ...
        $affectRow = 0;
        $affectRow = mysql_affected_rows($link);
