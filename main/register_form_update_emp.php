...
$mb_level       = isset($_POST['mb_level']) ? trim($_POST['mb_level']) : 4;
...
$mb_open        = isset($_POST['mb_open'])          ? trim($_POST['mb_open'])        : "1";
...
$mb_vaca_cnt    = isset($_POST['mb_vaca_cnt']) ? trim($_POST['mb_vaca_cnt']) : '0';
...
$obj = explode('?', $_SERVER['HTTP_REFERER']);
$ref = $obj[0];

if(basename($ref) == "employee_form"){
    // $mb_password    = isset($_POST['mb_id']) ? trim($_POST['mb_id']) : '';
    if($w == "") {
        $mb_password_re = isset($_POST['mb_password']) ? trim($_POST['mb_password']) : '';
    } else {
...
if (isset($_FILES['mb_icon']) && is_uploaded_file($_FILES['mb_icon']['tmp_name'])) {
    if (preg_match($image_regex, $_FILES['mb_icon']['name'])) {
        ...
        if ($_FILES['mb_icon']['size'] <= $config['cf_member_icon_size']) {
            ...
            alert('회원아이콘을 '.number_format($config['cf_member_icon_size']).'바이트 이하로 업로드 해주십시오.');
        }
    ...
    if (isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name'])) {
        ...
        if (preg_match($image_regex, $_FILES['mb_img']['name'])) {
            ...
            if ($_FILES['mb_img']['size'] <= $config['cf_member_img_size']) {
                ...
                //$msg .= '명함이미지을 '.number_format($config['cf_member_img_size']).'바이트 이하로 업로드 해주십시오.';
                alert('명함이미지을 '.number_format($config['cf_member_img_size']).'바이트 이하로 업로드 해주십시오.');
            }
...
if(basename($ref) == "employee_form"){
    goto_url($ref."?w=u&mb_id={$mb_id}&amp;{$qstr}");
}else{
    goto_url(G5_URL);
}