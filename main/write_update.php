...
if ($w == '' || $w == 'r') {
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if($bo_table !="design_req" && $member['mb_deptno'] != "9") {
    ...
    if($bo_table == "design_req" && $wr_9 == "" && $member['mb_deptno'] == "9") {
        $wr_9 = $member['mb_name'];
    }

    if($bo_table == "design_req" && $wr_5 == "디자인제작") {

        if($member['mb_deptno'] != "9" && $write['mb_id'] != $member['mb_id']) {
            alert("디자인 제작 수정은 작성자만 할수있습니다.");
        }
        ...
        $wr_7 = $resultOne['mb_deptno'];
        $wr_8 = $resultOne['mb_no'];
    ...
    //수정시 디자이너 + WR2 에 값이 있으면 디자이너가 확인함 안읽은 상태로 쪽지보내기
    if($bo_table == "design_req") {
        if($member['mb_deptno'] == "9" && $wr_2 != "") {
