				...
				<div class="col-12">
					...
					<div class="card card-danger">
						<div class="card-header">
							<h3 class="card-title"><?php echo $w == "" ? $board['bo_subject'].' 입력' : $board['bo_subject'].' 수정'?> </h3>
						</div>

						<form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" class="mt-2">
							...
							<input type="hidden" id="wr_5" name="wr_5" value="<?php echo $wr_5 ?>">
							<input type="hidden" id="wr_6" name="wr_6" value="<?php echo $wr_6 ?>">
							<input type="hidden" id="wr_7" name="wr_7" value="<?php echo $wr_7 ?>">
							<input type="hidden" id="wr_8" name="wr_8" value="<?php echo $wr_8 ?>">
							...
							<div class="form-group row ml-1">
								...
								<div class="wr_content col-sm-10 <?php echo $is_dhtml_editor ? $config['cf_editor'] : ''; ?>">
									...
									<?php } ?>
									<?php if (!$is_dhtml_editor) $editor_html = str_replace('<textarea id="wr_content" name="wr_content"', '<textarea id="wr_content" name="wr_content" class="form-control" cols="30" ', $editor_html); ?>
							...
							<?php if($bo_table == "design_req") { ?>

								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">마감요청일</label>

									<div class="col-sm-10">
										<div id="autosave_wrapper">
											<div class="input-group">
												<input type="date" name="wr_1" id="wr_1" class="form-control" value="<?php echo $wr_1 ?>" required >
											</div>
										</div>
									</div>
								</div>
								...
								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">작업시작일</label>

									<div class="col-sm-10">
										<div id="autosave_wrapper">
											<div class="input-group">
												<input type="date" name="wr_2" id="wr_2" class="form-control" value="<?php echo $wr_2 ?>" <?php echo $member['mb_deptno'] != "9"? "readonly" : "" ?>>
											</div>
										</div>
									</div>
								</div>
								...
								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">작업마감일</label>

									<div class="col-sm-10">
										<div id="autosave_wrapper">
											<div class="input-group">
												<input type="date" name="wr_3" id="wr_3" class="form-control" value="<?php echo $wr_3 ?>" <?php echo $member['mb_deptno'] != "9"? "readonly" : "" ?>>
											</div>
										</div>
									</div>
								</div>
									...
									<div class="col-sm-10">
										<div id="autosave_wrapper">
											<div class="input-group">
												...
												<select class="form-control" name="wr_4" id="wr_4" <?php echo $member['mb_deptno'] != "9"? "readonly onFocus='this.initialSelect = this.selectedIndex;' onChange='this.selectedIndex = this.initialSelect;'" : "" ?>>
													<option value="대기" <?php echo get_selected($wr_3, '대기'); ?>>대기</option>
													<option value="작업중" <?php echo get_selected($wr_3, '작업중'); ?>>작업중</option>
													<option value="작업완료" <?php echo get_selected($wr_3, '작업완료'); ?>>작업완료</option>
													<option value="작업반려" <?php echo get_selected($wr_3, '작업반려'); ?>>작업반려</option>
													<option value="반려" <?php echo get_selected($wr_3, '취소'); ?>>취소</option>
												</select>
											</div>
										</div>
									</div>
								...
								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">디자이너</label>
									<div class="col-sm-10">
										<div id="autosave_wrapper">
											<div class="input-group">
												<input type="text" name="wr_9" id="wr_9" class="form-control" value="<?php echo $wr_9 ?>" readonly>
											</div>
										</div>
									</div>
								</div>
							...
							<?php for ($i = 1; $is_link && $i <= G5_LINK_COUNT; $i++) { ?>
								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">링크<?php echo $i ?></label>

									<div class="col-sm-10">
										<input class="form-control" type="url" name="wr_link<?php echo $i ?>" value="<?php if ($w == "u") {
																															echo $write['wr_link' . $i];
																														} ?>" id="wr_link<?php echo $i ?>" placeholder="링크주소를 입력해주세요.">
									</div>
								</div>
							<?php } ?>
							...
							<?php for ($i = 0; $is_file && $i < $file_count; $i++) { ?>
								<div class="form-group row ml-1">
									<label class="col-sm-1 col-form-label">파일<?php echo $i + 1 ?></label>
									...
									<div class="col-sm-10">
										<div class="input-group">
											<div class="custom-file">
												...
												<label class="custom-file-label" for="bf_file_<?php echo $i + 1 ?>"><?php if ($w == 'u' && $file[$i]['file']) echo $file[$i]['source'];
																													else echo '파일을 선택해주세요.'; ?></label>
											</div>
...
<script>
	...
	function fwrite_submit(f) {
		...
		if(wr_5 == "" && wr_4 == "작업중" ) {
			var result = confirm("해당 요건의 디자인 제작을 생성하겠습니까?");
